...
import ImageSearchItems from "@/components/ImageSearchItems";
...
export default async function ImageSearch({
  ...
  const response = await fetch(
    `https://www.googleapis.com/customsearch/v1?key=${GOOGLE_API_KEY}&cx=${GOOGLE_CONTEXT_KEY}&q=${searchTerm}&searchType=image`
  );
  ...
  const { items }: { items: SearchDataImageType[] } = data as {
    items: SearchDataImageType[];
  };
